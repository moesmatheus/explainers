  ...
  return (
    <div className="rounded-xl bg-black/40 border border-white/10 p-4">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" onMouseLeave={() => setHov(null)}>
        ...
        {REFS.map((r, i) => {
          const cx = xOf(r.v);
          // clamp anchor near edges; stagger desc into two rows to avoid horizontal collisions
          ...
          return (
            <g key={r.lab}
              ...
              {/* connector tick from ref line down to desc row when staggered */}
...
const PowerInfrastructure = () => {
  ...
  return (
    <Card id="power" icon={Zap} title="Power infrastructure" subtitle="From 138 kV to 12 V — and every joule of loss along the way" accent="amber" index={4}>
      <p>
        The single largest engineering challenge in a DC is <strong>delivering clean, uninterrupted power at scale</strong>. A 30 MW facility pulls more than a small city's worth of electricity at a single meter, and every component from the utility interconnect to the server PSU is designed around two principles: <em>no single failure takes the IT load down</em>, and <em>every watt of loss has to be cooled</em>. The flow looks like a funnel with built-in redundancy at each stage.
      </p>
...
// ordered by latency so the alternating above/below label pattern never collides
...
export default function DataCentersExplainer() {
  return (
